...
class imsbr_tuple_parser(imsbr_parser):
	...
	def checkit(self, t, m):
		...
		add_line_to_file(t +"  "+m.group(1) + " " + m.group(2) + m.group(3),'imsbr.txt', 0) 
...
class imsbr_callstate_parser(imsbr_parser):
	...
	def checkit(self, t, m):
		add_line_to_file(t + "  Start " + m.group(1) + " call", 'imsbr.txt', 0) 
...
class imsbr_handover_parser(imsbr_parser):
	...
	def checkit(self, t, m):
		ho = m.group(1)
		add_line_to_file(t + "  "+ "Trigger" + "  "+ho, 'imsbr.txt', 0) 
		if ho == "ho-finish":
			...
			add_line_to_file( t+ "  "+  "!!!---Handover packets overview: fromcp=%d, tocp=%d" %(fromcp, tocp), 'imsbr.txt', 0) 

			if imsbr_parser.last_ho_state == "ho-lte2wifi":
				if fromcp == 0:
					add_line_to_file( "@@@@NO PACKETS FROM CP, NEED CP CHECK IT FIRST@@@@", 'imsbr.txt', 0) 
				elif tocp == 0:
					add_line_to_file( "@@@@NO PACKETS RECV FROM WIFI, MAYBE IP CHANGED, SECURITY CHECK IT FIRST@@@@", 'imsbr.txt', 0) 

		elif ho == "ho-wifi2lte":
			if imsbr_parser.vowifi_tuples < 1:
				add_line_to_file( "@@@@NO VOWIFI TUPLES ADDED, BUT HANDOVER FROM VOWIFI TO VOLTE@@@@", 'imsbr.txt', 0)
		elif ho == "ho-lte2wifi":
			if imsbr_parser.volte_tuples < 1:
				add_line_to_file(  "@@@@NO VOLTE TUPLES ADDED, BUT HANDOVER FROM VOLTE TO VOWIFI@@@@", 'imsbr.txt', 0)
