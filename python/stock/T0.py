# -*- coding: utf-8 -*-
"""
Created on Wed Sep 16 06:34:03 2015
...
def calc_dadan_num(df, am_level):       
    max_p = df.irow(0).price
    max_p_time = df.irow(0).time
    min_p = df.irow(0).price
    min_p_time = df.irow(0).time
    up = True
    for i in range(0, len(df)):
        if (df.irow(i).price > max_p):
            max_p = df.irow(i).price
            max_p_time = df.irow(i).time
            up = True
        if (df.irow(i).price < min_p):
            min_p = df.irow(i).price
            min_p_time = df.irow(i).time
            up = False
        
        if (max_p > min_p *1.02 and up):
            print "%s %.2f -> %s %.2f"%(min_p_time,min_p,max_p_time, max_p)
            min_p = 500
        if (max_p > min_p *1.02 and not up):
            print "%s %.2f -> %s %.2f"%(max_p_time, max_p, min_p_time, min_p)
            max_p = 0
