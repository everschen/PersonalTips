...
def calc_sell_num(code, price_buy, price_sell,debug,df):    
    
    bought = False
    sell_num = 0
    date_info=""
    for i in range(len(df)-1, 0, -1):
        if (not bought and df.irow(i).price < price_buy):
            date_info=date_info+df.irow(i).time+"->"
            bought = True
        elif (bought and df.irow(i).price > price_sell):
            date_info=date_info+df.irow(i).time+"\n"
            bought = False
            sell_num = sell_num +1
    if (debug):
...
price_low = float(aa.irow(0).low)
price_high = float(aa.irow(0).high)

df = get_today_ticks(code)
#df.irow(i).type.decode('utf-8')=="卖盘"decode('UTF-8') and 
print df.irow(0).type
print "\n"
for i in floatrange(price_low, price_high, 0.05):
    buy = round(i,2)
    sell = round(buy+buy*0.03,2)
    nums=calc_sell_num(code,i,sell,False,df)
