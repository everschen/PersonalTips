...
usdt_amount = 20
if argnum >= 3:
    usdt_amount = int(sys.argv[2])

diff_space_buy = 35
if argnum >= 4:
    diff_space_buy = int(sys.argv[3])

diff_space_sell = 30
if argnum >= 5:
    diff_space_sell = int(sys.argv[4])

deal_space = 50  # sell out if the space reach 50?
if argnum >= 6:
    deal_space = int(sys.argv[5])
...
def handle_order_id(order_id, order_type, price):
    ...
    if order_type=="sell-limit":
        ...
        output_to_log_file(f_res, "SELL done: %.2f, buy:%.2f diff:%.2f"%(price, buy_price, price-buy_price))
        sell_order_ongoing = False
    elif order_type=="buy-limit":
        remove_order_in_db(order_id)
        price_list.append(price)
        output_to_log_file(f_res, "BUY done: %.2f, %r"%(price, price_list))
        buy_order_ongoing = False
    else:
...
def loop_handle():
    ...
    global deal_space  # sell out if the space reach 50?
    ...
    while (1):
        ...
        if q_order == 1:
            ...
            if not sell_order_ongoing and (current_price-buy_price) > diff_space_sell and buy_price:
            ...
            if not sell_order_ongoing and current_price - buy_price > deal_space and buy_price:
                #output_to_log_file(f_res, "SELLQ: %.2f, deal_space=%.2f buy:%.2f diff:%.2f"%(current_price, deal_space, buy_price, current_price-buy_price))
                ret = handle_sell_order(symbol, current_price, usdt_amount)
                if ret:
                    sell_order_ongoing = True
                    selling = current_price
        ...
        if eq_order == 1:
            ...
            eqstr = "                  "
            if not sell_order_ongoing and current_price-buy_price > deal_space and buy_price:
                #output_to_log_file(f_res, "SELLEQ: %.2f, deal_space=%.2f buy:%.2f diff:%.2f"%(current_price, deal_space, buy_price, current_price-buy_price))
                ret = handle_sell_order(symbol, current_price, usdt_amount)
                if ret:
                    sell_order_ongoing = True
                    selling = current_price
        ...
        printme("gap:%s sell/deal=%d/%d buy_space=%d usdt:%d price list: %r"%(showgap, diff_space_sell, deal_space, diff_space_buy, usdt_amount, price_list))
        #if qstr or eqstr:
        printme("==================================================================================================================\n")
        lock.release()
