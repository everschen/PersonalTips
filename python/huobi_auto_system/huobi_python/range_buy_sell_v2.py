...
def create_my_order(buy_price, sell_price, symbol, each_usdt_amount):
    ...
    if (trade_bal > sell_eth_amount or trade_bal>0.2) and sell_price > current_price and last_state_price ==1:
        #only after buy then can sell, so the last state should be 1 if want to sell
        if trade_bal < sell_eth_amount and trade_bal>0.2:
            printme_warning("something like less balance now, what happened? maybe deduct fee cause this issue. trade_bal=%f sell_eth_amount=%f, updated!"%(trade_bal, sell_eth_amount))
            sell_eth_amount = trade_bal
        printme("There are balacne %f of %s, sell %f at price %f first."%(trade_bal, symbol, sell_eth_amount, sell_price))
        ...
        printme_warning("IF not price is low than buy price, then please have a check, trade_bal=%f, sell_eth_amount=%f, current_price=%f, buy_price=%.4f, sell_price=%.4f\n"%(trade_bal, sell_eth_amount, current_price, buy_price, sell_price))
...
def update_order_implement(trigger):
    ...
    for buy_price in price_points:
        ...
        printme("[%s] handle price %.4f <-- %f (current price)"%(trigger, buy_price, current_price), True)
        ...
        if execute_result and execute_num:
            ...
            if has_exact_order and is_buy:
                ...
                printme("[%s] Has BUY order %s ongoing, %.4f <-- %f (current price) waiting..."%(trigger, orderid, buy_price, current_price))
                state_points[buy_price] = 1
                ...
                printme("[%s] Has SELL order %s ongoing, %.4f <-- %f (current price) waiting..."%(trigger, orderid, sell_price, current_price))
                state_points[buy_price] = 2
...
def handle_order_id(order_id, order_type, price):
    ...
    if order_type=="sell-limit":
        ...
        if trade_fee_deduct_from_symbol:
            total_num = int((each_amount*(1-trade_fee_deduct_value)/buy_price)*amount_precision)/amount_precision
        else:
            total_num = int((each_amount/buy_price)*amount_precision)/amount_precision
...
def start_to_buy_sell(is_short, price):
    argnum = len(sys.argv)
    if argnum < 4:
    ...
    for i in range(0, int(sys.argv[3])):
        buy_price_tmp = int((start_point + i*step)*price_precision)/price_precision
...
if __name__ == '__main__':
    ...
    while 1:
        #check price
        #if buy the ganrantee then start handle buy and sell and quit the while
        ret, is_short, price = check_insurance_point()
        if ret == True:
            start_to_buy_sell(is_short, price)
            break
        time.sleep(3)
