...
def update_order_implement(trigger):
    ...
    for buy_price in price_points:
        ...
        printme("[%s] handle price %.4f <-- %f (current price)"%(trigger, buy_price, current_price))
        ...
        if execute_result and execute_num:
            ...
            if has_exact_order and is_buy:
                ...
                printme("[%s] Has BUY order %s ongoing, %.4f <-- %f (current price) waiting...\n"%(trigger, orderid, buy_price, current_price))
                state_points[buy_price] = 1
                ...
                printme("[%s] Has SELL order %s ongoing, %.4f <-- %f (current price) waiting...\n"%(trigger, orderid, sell_price, current_price))
                state_points[buy_price] = 2
    ...
    printme("each_amount = %f, current_prince=%f, buy price:%r, Let me sleep for one minute..."%(each_amount, current_price, price_points))
