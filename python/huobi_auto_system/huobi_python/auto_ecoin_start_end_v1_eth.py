...
def print_current_price_buy_sell_state():
    ...
    for k,v in state_points.items():
        if v==1:
            ...
            print("%.4f  %s  -->  %.4f"%(k,"SELL", k + price_diff))
        if v == 1:
            buy_num = buy_num +1
        elif v == 2:
            sell_num = sell_num +1
...
def create_my_order(buy_price, sell_price, symbol, eth_amount):
    ...
    if (eth3_bal > eth_amount or eth3_bal>0.2) and sell_price > current_price and last_state_price ==1:
        #only after buy then can sell, so the last state should be 1 if want to sell
        if eth3_bal < eth_amount and eth3_bal>0.2:
            logger.warning("something like less balance now, what happened? maybe deduct fee cause this issue. eth3_bal=%f eth_amount=%f, updated!"%(eth3_bal, eth_amount))
            eth_amount = eth3_bal
        logger.info("There are balacne %f of %s, sell %f at price %f first."%(eth3_bal, symbol, eth_amount, sell_price))
        total_sell_num = int(eth_amount*10000)/10000
        ...
        if total_usdt < 1
            logger.warning("total usdt is very less, it's %f, can't buy now, please check your usdt balance"%(total_usdt))
            return False

        use_usdt = eth_amount * buy_price

        logger.info("total usdt is %f, use_usdt=%f, buy_price=%.4f, eth_amount=%f"%(total_usdt, use_usdt, buy_price, eth_amount))

        logger.info("use usdt %f to buy %f %s at price %.4f now..."%(use_usdt, eth_amount, symbol, buy_price))
        ret, max_size = create_buy_order(symbol, buy_price, eth_amount)
        if ret:
...
if argnum == 5:
    eth_amount = float(sys.argv[4])

while 1:
    print("\n\n\n")
    current_price = get_current_price(symbol)
    for buy_price in price_points:
        sell_price = buy_price + price_diff
        ...
        if execute_result and execute_num:
            ...
            if has_exact_order and is_buy:
                ...
                created_success = create_my_order(buy_price, sell_price, symbol, eth_amount)
    ...
    logger.info("eth_amount = %f, current_prince=%f, buy price:%r, Let me sleep for one minute..."%(eth_amount, current_price, price_points))
