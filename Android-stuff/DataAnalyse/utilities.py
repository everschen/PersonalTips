...
def write_buffer_to_file(filename):
	#print len(bufferlines)
	if len(bufferlines) == 0:
		return
	file_object = open(filename, 'a+')
	file_object.writelines(bufferlines)
	file_object.close()


def add_line_to_buffer(source, bufferlines, insert, is_mtc):
	str_len=18

	if source in bufferlines: # source already existed in the target file
		return
	...
	if len(bufferlines) == 0 : #target file is empty
		bufferlines.append(source)
		return
	...
	for i in range(0, len(bufferlines)): 
		if cmp(bufferlines[i][0:str_len],source[0:str_len]) > 0 :
			bufferlines.insert(i,source)
			return
		
	#print source, is_mtc, insert, filename, len(lines)
	if len(bufferlines)-1 == i : #source time stamp is latest
		bufferlines.append(source)
		return	
			...
			if ret != "" :
				...
				if (delta.seconds > 15):
					...
					if (last.value.lower() != current.value.lower()):
						...
						add_line_to_buffer(last.source, bufferlines, insert_mode, is_mtc)
						#add_line_to_files(current.source,output,sep, insert_mode, is_mtc)
						add_line_to_buffer(current.source, bufferlines, insert_mode, is_mtc)
...
def reg_filter_pattern(sep, res_pattern, reg_pattern, source, output_file, insert_mode, is_mtc, trans_sep):		
	...
	for pat in pats:
		...
		if p.match(source):
			...
			add_line_to_buffer(source, bufferlines, insert_mode, is_mtc)
			return 0
	return 2
...
def filter_file(section, input_file, output, sep, filter, case_sensitive, restrict_filter, regular_filter, insert_mode, equal_value,tran_sep):	
	#print section, input_file, output, sep, filter, case_sensitive, restrict_filter, regular_filter, insert_mode, equal_value,tran_sep
...
def remove_output(cf, section):
	outputfile = cf.get(section, 'output').replace("\n", "")
	if os.path.isfile(outputfile):
		os.remove(outputfile)
		print "Remove output file:",outputfile

def handle_main_section(cf):
	del bufferlines[:]
	secs = cf.sections()
	#print 'sections:', secs	 

	main_filter = cf.get('main', "filter").replace("\n", "")
	#print main_filter
	...
	for support_opt in options:
		if support_opt in opts:
			Val[support_opt] = cf.get('main', support_opt).replace("\n", "")
	...
	Val['filter'] = Val['filter_sep'].join(pats)
	
	filter_file('main',Val['input'],Val['output'],Val['filter_sep'],Val['filter'],Val['case_sensitive'],
		Val['restrict_filter'],Val['regular_filter'],Val['insert_mode'],Val['equal_value_check'],Val['translate_sep'])
	#print len(bufferlines)
	write_buffer_to_file(Val['output'])
